...
<div id="articles-list-page">
    <div>
        ...
        <div v-else>
            <p><a href="#" @click="checkAll">Выделить всё</a> | <a href="#" @click="uncheckAll">Снять все выделения</a></p>
            
            <p><hr><button class="btn btn-primary" @click="exportArticles" :disabled="!statusButtonExport">Экспортировать записи</button></p>
            <div id="wz_progress" >
            ...
            <table  id="list-tasks" class="wp-list-table widefat fixed striped posts">
                ...
                <tbody>
                    <tr v-for="article in articles" :key="article.id">
                        <td><input ref="selectArticle" type="checkbox" :value="article.id" :id="article.id" v-model="selectedArticles"></td>
                        <td>{{ article.name }}</td>
                        <td>
                            <p ref="articleEdit"><router-link  :to="'/article-edit/' + selectedPermissionFolders.toFolderID + '/' + selectedPermissionFolders.toFolderName + '/' + article.id+ '/' + selectedPermissionFolders.categoryID">Перейти</router-link></p>
                        </td>
                    </tr>
                </tbody>
            </table>
...
function wz_workhard_ajax_articles() {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        if (wzz_validate_params(array('method', 'from_folder_id')) && $_GET['method'] == 'fetch_articles') {
            ...
            foreach($folders['response'] as $folder) {
                if ($folder['id'] == $_GET['from_folder_id']) {
                    ...
                    $result = Arrch::find($folder['tasks'], $options);
            ...
            wp_die();
        } elseif (wzz_validate_params(array('method', 'task_id')) && $_GET['method'] == 'fetch_article') {
            /* Получить статью */
            $token = get_option('wz_options_workhard')['token'];
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        if (wzz_validate_params(array('method', 'order_name',  'text', 'category_id')) && $_POST['method'] == 'insert_post') {
        ...
        } elseif (wzz_validate_params(array('method', 'folder_id',  'task_id')) && $_POST['method'] == 'move_task_to_folder') {
        ...
        } elseif (wzz_validate_params(array('method', 'folder_name',  'order_name', 'post_url', 'price', 'total_price')) && $_POST['method'] == 'insert_spreadsheet_user') {
